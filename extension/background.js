...
async function reportTrackerStatus(settings) {
  ...
  try {
    await fetch(`${normalizeBase(settings.backendBase)}/api/proxy.php?path=${encodeURIComponent(`/tracker-status/${settings.childId}`)}`, {
      method: 'PUT',
      headers: {
...
function ensurePolicyAlarm() {
  ...
  chrome.alarms.create('snaillyPolicyRecheck', { periodInMinutes: 0.1 });
}
...
async function trackTab(tab, reason = 'tab_event') {
  ...
  if (isRecentDuplicate(tab.id || 0, url) && !settings.blockDangerous) {
    return { ok: true, skipped: true, message: 'Duplicate ignored.' };
  }

  try {
    const response = await fetch(`${settings.backendBase}/api/track.php`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'X-Snailly-Authorization': `Bearer ${settings.token}`
...
chrome.tabs.onUpdated.addListener((tabId, changeInfo, tab) => {
  if (changeInfo.url || changeInfo.status === 'complete') {
    trackTab({ ...tab, id: tabId }, changeInfo.url ? 'url_changed' : 'page_complete');
  }
});
